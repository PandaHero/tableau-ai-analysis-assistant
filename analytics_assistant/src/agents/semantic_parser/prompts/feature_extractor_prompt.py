...
def build_feature_extractor_prompt(
    ...
    if current_step_intent:
        step_goal = current_step_intent.goal or current_step_intent.purpose or current_step_intent.title
        lines.append(f"当前步骤: {current_step_intent.title}")
        lines.append(f"步骤目标: {step_goal}")
        if current_step_intent.semantic_focus:
            lines.append(f"语义重点: {', '.join(current_step_intent.semantic_focus[:5])}")
        if current_step_intent.candidate_axes:
            lines.append(f"候选维度/解释轴: {', '.join(current_step_intent.candidate_axes[:5])}")
        if current_step_intent.depends_on:
            lines.append(f"依赖步骤: {', '.join(current_step_intent.depends_on)}")

    if evidence_context and evidence_context.step_artifacts:
        last_artifact = evidence_context.step_artifacts[-1]
        if last_artifact.table_summary:
            lines.append(f"前序证据: {last_artifact.table_summary}")
