...
from ..components.history_manager import HistoryManager, estimate_tokens
...
- 澄清问题（clarification_question）应使用通俗易懂的自然语言，面向业务用户而非技术人员
- 澄清选项（clarification_options）应使用字段的中文显示名或业务别名，避免使用技术字段名（如 SUM(Sales)）
</rules>
...
GLOBAL_UNDERSTANDING_ENHANCEMENT = '''<global_understanding_task>
当前问题被初步识别为复杂查询（{rule_plan_mode}）。你需要在输出中额外填写以下全局理解字段：

1. analysis_mode: 分析模式判断
   - single_query: 简单直接单查
   - complex_single_query: 语义复杂但仍可由单条查询完成
   - multi_step_analysis: 需要多步拆解，下一步依赖前一步结果
   - why_analysis: 原因/归因分析

2. single_query_feasible: 是否可以通过单条查询表达
   - 如果问题复杂但 Tableau 仍能单查，使用 complex_single_query + single_query_feasible=true
   - 不要为了保守而强行拆步

3. decomposition_reason: 为什么需要拆解或可以保留单查的原因说明

4. risk_flags: 分析中发现的风险点（口径不清、基线缺失等）

注意：同时仍然要完整填写 what/where/computations/self_check 等标准语义解析字段。
即使判断为 multi_step 或 why_analysis，仍要先输出覆盖主问题首跳的查询骨架。
</global_understanding_task>'''
...
class DynamicPromptBuilder:
    ...
    def build(
        ...
        # 9. 复杂查询注入全局理解增强指令
        global_understanding_section = ""
        if analysis_plan and analysis_plan.needs_planning:
            rule_plan_mode = PLAN_MODE_NAMES.get(
                analysis_plan.plan_mode, analysis_plan.plan_mode.value
            )
            global_understanding_section = GLOBAL_UNDERSTANDING_ENHANCEMENT.format(
                rule_plan_mode=rule_plan_mode,
            )

        # 10. 组装
        parts = [header, task_section]
        if global_understanding_section:
            parts.append(global_understanding_section)
        parts.append(footer)
        base_prompt = "\n\n".join(parts)
    ...
    def _format_history(self, history: Optional[list[dict[str, str]]]) -> str:
        ...
        if not history:
            return ""
        return self._history_manager.format_history_for_prompt(history)
