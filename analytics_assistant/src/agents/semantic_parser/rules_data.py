...
预置规则模式用于：
- IntentRouter: 无关问题检测（正则表达式模式）

规则模式是领域知识，类似种子数据，不应放入 app.yaml 配置文件。
配置文件只存放阈值、超时等运行时参数。

用法：
    from analytics_assistant.src.agents.semantic_parser.rules_data import (
        IRRELEVANT_PATTERNS,
        get_irrelevant_patterns,
    )
"""
