...
- **new_question**: 新的主后续问题（自然语言，should_replan=true 时必须非空）
- **suggested_questions**: 兼容旧协议的建议问题列表，可与 `candidate_questions` 保持一致
- **candidate_questions**: 结构化候选问题列表，必须包含 `question / question_type / priority / expected_info_gain / rationale / estimated_mode`

## 约束
- 必须：should_replan=true 时 new_question 非空
- 必须：reason 详细说明决策依据
- 必须：candidate_questions 至少提供 1-3 个高价值候选问题，并按 priority 从高到低排序
- 必须：expected_info_gain 取值在 0-1 之间，表示继续分析的预期收益
- 必须：estimated_mode 明确下一轮更像 single_query / complex_single_query / multi_step_analysis / why_analysis 哪一种
- 建议：suggested_questions 与 candidate_questions 中的问题文本保持一致，方便兼容旧链路
