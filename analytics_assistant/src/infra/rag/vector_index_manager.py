...
# 先定义 logger，避免在 FAISS 导入失败时出现 NameError
logger = logging.getLogger(__name__)

try:
    # 抑制 FAISS 加载时的 INFO 级别日志（如 "Loading faiss with AVX2 support"）
    ...
    logger.warning("FAISS 未安装，将使用简单向量搜索。安装命令: pip install faiss-cpu")
...
class VectorIndexManager:
    ...
    def __init__(
        ...
        Args:
            embedding_provider: Embedding 提供者（LangChain Embeddings 实例，默认使用 ModelManager）
            index_config: 索引配置
            datasource_luid: 数据源 LUID
            index_dir: 索引存储目录
            use_cache: 是否使用向量缓存（注意：缓存功能需要在阶段 1.3 实现）
        """
        ...
        # 初始化 Embedding 提供者
        # 优先使用传入的 provider，否则尝试从 ModelManager 获取默认 Embedding
        if embedding_provider is not None:
            self.embedding_provider = embedding_provider
            self._rag_available = True
        else:
            self.embedding_provider = self._create_default_embedding_provider()
            self._rag_available = self.embedding_provider is not None
        
        # TODO: 缓存功能将在阶段 1.3 实现（使用 LangGraph SqliteStore）
        if use_cache and self.embedding_provider is not None:
            logger.debug("Embedding 缓存功能将在阶段 1.3 实现")
    ...
    def _create_default_embedding_provider(self) -> Optional[Any]:
        ...
        使用 ModelManager 的 get_embeddings() 获取默认 Embedding 模型。
        
        Returns:
            LangChain Embeddings 实例，或 None（表示应回退到 LLM）
        """
        try:
            # 使用相对导入（正确的做法）
            ...
            logger.debug("使用 ModelManager 的默认 Embedding 提供者")
            return embeddings
        except Exception as e:
            logger.warning(f"初始化默认 Embedding 提供者失败: {e}")
            return None
    ...
    def _incremental_update(self, changed_fields: List[Any]) -> int:
        ...
        logger.info(f"增量更新完成: 更新了 {len(changed_fields)} 个字段")
        return len(self._chunks)
    ...
    def _simple_search(self, query_vector: List[float]) -> List[Tuple[str, float, float]]:
        ...
        Returns:
            (field_name, confidence, raw_score) 列表，按相似度降序排序
