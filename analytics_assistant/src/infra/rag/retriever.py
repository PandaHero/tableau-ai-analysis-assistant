...
class EmbeddingRetriever(BaseRetriever):
    ...
    封装 FAISS 向量检索，使用 VectorIndexManager 进行检索。
    ...
    def retrieve(
        ...
        # 使用 VectorIndexManager 进行搜索
        search_k = k * 2 if filters else k
        
        results = self.vector_index_manager.search(
            query=query,
            top_k=search_k,
...
class KeywordRetriever(BaseRetriever):
    ...
    def _build_bm25_index(self) -> None:
        """构建 BM25 索引"""
        chunks = self.vector_index_manager.get_all_chunks()
        if not chunks:
            return
    ...
    def retrieve(
        ...
        for rank, (field_name, score) in enumerate(scored_fields, 1):
            ...
            chunk = self.vector_index_manager.get_chunk(field_name)
            if chunk:
...
class RetrieverFactory:
    ...
    def create_embedding_retriever(
        ...
        """创建向量检索器"""
        return EmbeddingRetriever(vector_index_manager, config)
    ...
    def create_keyword_retriever(
        ...
        """创建关键词检索器"""
        return KeywordRetriever(vector_index_manager, config)
    ...
    def create_hybrid_retriever(
        ...
        """创建混合检索器"""
        embedding_retriever = EmbeddingRetriever(vector_index_manager, config)
        keyword_retriever = KeywordRetriever(vector_index_manager, config)
    ...
    def create_pipeline(
        ...
        **kwargs
        ...
        if retriever_type == "embedding":
            retriever = RetrieverFactory.create_embedding_retriever(vector_index_manager, config)
        elif retriever_type == "keyword":
            retriever = RetrieverFactory.create_keyword_retriever(vector_index_manager, config)
        else:  # hybrid
            retriever = RetrieverFactory.create_hybrid_retriever(
                vector_index_manager, config,
