...
3. **配置驱动**：所有外部依赖（LLM、Tableau API）的配置信息已在 `.env` 文件中配置
4. **测试完整性**：测试问题必须完整解决，不能跳过测试，不能忽略失败的测试
5. **错误处理**：所有代码必须有完整的错误处理和日志记录
6. **代码质量**：代码必须符合生产标准，包括类型注解、文档字符串、合理的抽象

**测试环境配置**：
- LLM 配置：`.env` 文件中的 `LLM_*` 相关配置
- Tableau 配置：`.env` 文件中的 `TABLEAU_*` 和 `VDS_*` 相关配置
- 测试数据源：使用真实的 Tableau 数据源进行测试
...
- **Component（组件）**: 不暴露给 LLM 的代码模块，代码直接调用
...
- **LOD（Level of Detail）**: 详细级别表达式，用于不同粒度的聚合计算

### 数据模型
- **VizQLState**: 工作流状态，贯穿整个 StateGraph 的状态定义
- **SemanticQuery**: Understanding Agent 输出的纯语义查询模型，不包含 VizQL 技术概念
- **MappedQuery**: FieldMapper Node 输出，业务术语已映射为技术字段名
- **VizQLQuery**: QueryBuilder Node 输出的技术查询，包含 VizQL 表达式
- **QueryResult**: Execute Node 输出的查询结果
- **AnalysisSpec**: 分析规格，描述派生计算（累计、排名、占比等）
- **ReplanDecision**: Replanner Agent 输出的重规划决策
...
**⚠️ 实现注意**：LangChain 中间件 API 可能因版本不同而有差异。实现前需验证：
1. 确认 `langchain.agents.middleware` 模块在当前 LangChain 版本中存在
2. 如果 API 不存在或不兼容，需要自主实现等效功能
3. 参考 LangGraph 的 `CompiledStateGraph.with_config()` 或自定义中间件模式
...
**说明**：采用纯语义中间层架构，移除 Planning Agent 和 Boost Agent。原 Boost Agent 的功能合并到 Understanding Agent：
- **Understanding Agent**（LLM Agent）：问题分类 + 语义理解，输出 SemanticQuery
- **FieldMapper Node**（RAG + LLM 混合节点）：语义字段映射，独立节点
- **QueryBuilder Node**（纯代码节点）：ImplementationResolver + ExpressionGenerator
- **Execute Node**（纯代码节点）：VizQL API 调用

**架构变更**：
```
旧架构: Boost → Understanding → Planning → Execute → Insight → Replanner
新架构: Understanding → FieldMapper → QueryBuilder → Execute → Insight → Replanner
             ↓               ↓              ↓
       SemanticQuery    MappedQuery    VizQLQuery
       (问题分类+语义)   (RAG+LLM)      (纯代码)
```

**Boost 功能合并说明**：
- **元数据获取**：Understanding Agent 调用 `get_metadata` 工具获取字段信息
- **问题分类**：Understanding Agent 在理解问题时判断是否为分析类问题（is_analysis_question）
- **非分析类问题处理**：如果 is_analysis_question=False，直接路由到 END 并返回友好提示

**节点类型说明**：
| 节点 | 类型 | 说明 |
|------|------|------|
| Understanding | LLM Agent | 问题分类 + 语义理解，输出 SemanticQuery |
| FieldMapper | RAG + LLM 混合 | 语义字段映射，RAG 检索 + LLM fallback |
| QueryBuilder | 纯代码 | ImplementationResolver + ExpressionGenerator |
| Execute | 纯代码 | VizQL API 调用 |
| Insight | LLM Agent | 数据洞察分析 |
| Replanner | LLM Agent | 重规划决策 |

#### 验收标准

1. 当创建工作流时，StateGraph 应定义以下节点：Understanding、FieldMapper（RAG+LLM 混合）、QueryBuilder（纯代码）、Execute（纯代码）、Insight、Replanner
2. 当工作流开始执行时，StateGraph 应遵循以下顺序：Understanding → FieldMapper → QueryBuilder → Execute → Insight → Replanner
3. 当 Understanding 判断问题不是分析类问题（is_analysis_question=False）时，StateGraph 应直接路由到 END 并返回友好提示
...
7. 当 FieldMapper 节点执行时，应使用 RAG 检索候选字段，置信度 < 0.9 时使用 LLM 判断
8. 当 QueryBuilder 节点执行时，应依次调用 ImplementationResolver、ExpressionGenerator 组件（纯代码）
9. 当 Understanding Agent 执行时，应调用 get_metadata 工具获取字段信息（原 Boost 功能）
...
4. 当注册工具时，Tool_System 应按 Agent 节点分组工具（understanding_tools、replanner_tools 等）
5. 当工具返回大输出（>20000 tokens）时，Tool_System 应自动保存到文件系统并返回文件引用


### 需求 4: FieldMapper 节点（RAG + LLM 混合）

**用户故事:** 作为开发者，我希望语义字段映射作为独立节点，以便清晰分离语义理解和字段映射职责。

**优先级**: P0（核心功能）

**说明**：FieldMapper 是独立的工作流节点，采用 RAG + LLM 混合模式：
- **RAG 快速路径**：置信度 >= 0.9 时直接返回，无需 LLM
- **LLM Fallback**：置信度 < 0.9 时使用 LLM 从候选字段中选择最佳匹配

**架构关系**：
```
Understanding Node
    ↓ SemanticQuery（业务术语）
FieldMapper Node（RAG + LLM 混合）
    ├── SemanticMapper.search()  ← RAG 检索
    ├── 置信度 >= 0.9? → 直接返回
    └── 置信度 < 0.9? → LLM 判断
    ↓ MappedQuery（技术字段）
...
**与旧设计的区别**：
- 旧设计：FieldMapper 是 QueryBuilder 内部组件，semantic_map_fields 是工具
- 新设计：FieldMapper 是独立节点，直接调用 SemanticMapper，无需工具封装

#### 验收标准

1. 当 FieldMapper 节点执行时，应接收 SemanticQuery 并输出 MappedQuery（业务术语已映射为技术字段）
2. 当执行字段映射时，FieldMapper_Node 应使用 SemanticMapper 进行向量检索
3. 当向量搜索 top-1 置信度 >= 0.9 时，FieldMapper_Node 应跳过 LLM 判断直接返回（快速路径）
4. 当向量搜索 top-1 置信度 < 0.9 时，FieldMapper_Node 应使用 LLM 从 top-k 候选中选择最佳匹配
5. 当映射置信度低于 0.7 时，FieldMapper_Node 应返回带置信度分数的 top-3 备选项供后续处理
6. 当执行字段映射时，FieldMapper_Node 应将结果缓存到 SQLite，TTL 为 1 小时
7. 当请求批量映射时，FieldMapper_Node 应使用 asyncio 并发处理最多 5 个术语
8. 当构建字段索引时，FieldMapper_Node 应包含维度层级信息（category、level、granularity）以增强检索准确性
9. 当返回映射结果时，FieldMapper_Node 应包含字段的维度层级信息（如 category="geographic"、level=2、granularity="province"）
...
**用户故事:** 作为开发者，我希望 Agent 可以通过工具访问数据源元数据，以便在 Understanding 阶段获取字段信息。
...
**说明**：Execute Node 是非 LLM 节点，直接调用 VizQL API，不通过工具系统。QueryBuilder Node 也是非 LLM 节点，内部调用 ImplementationResolver、ExpressionGenerator 组件。FieldMapper 是独立的前置节点。

1. 当 Execute 节点执行查询时，Execute_Node 应直接调用 VizQL Data Service /query-datasource API（不通过工具）
2. 当查询执行成功时，Execute_Node 应返回包含数据、行数和执行时间的 QueryResult
3. 当查询执行失败时，Execute_Node 应返回包含错误代码和消息的结构化错误
4. 当查询结果较大时，Execute_Node 应返回完整结果（VizQL API 不支持分页），并依赖 FilesystemMiddleware 处理大结果
...
5. 当执行字段映射时，FieldMapper_Node 应使用 SemanticMapper 进行 RAG 检索
6. 当 RAG 置信度 >= 0.9 时，FieldMapper_Node 应直接返回结果（快速路径，无需 LLM）
7. 当 RAG 置信度 < 0.9 时，FieldMapper_Node 应使用 LLM 从 top-k 候选中判断最佳匹配

**ImplementationResolver 组件（纯代码，在 QueryBuilder Node 内）：**
...
2. 当注册工具时，Tool_Registry 应按 Agent 节点分组：understanding_tools（get_metadata、get_schema_module、parse_date、detect_date_format）、replanner_tools（write_todos，来自 TodoListMiddleware）
3. 当 Agent 节点创建时，Tool_Registry 应根据节点类型自动注入对应的工具集
4. 当工具依赖其他服务（如 MetadataManager、DateManager）时，Tool_Registry 应支持依赖注入
...
- **FieldMapper Node**：是独立节点（RAG + LLM 混合），不是工具，直接调用 SemanticMapper
- **Boost 功能合并**：get_metadata 工具现在由 Understanding Agent 调用（原属于 Boost Agent）
...
8. 当没有洞察结果时，Replanner_Agent 应设置 completeness_score=0.0、should_replan=False、reason="没有洞察结果，无法评估完成度"，并路由到 END
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  R14 (工具注册) ─┬─> R5 (元数据工具) ← 封装 MetadataManager                   │
│                 ├─> R6 (日期工具) ← 封装 DateManager                         │
│                 └─> R7 (查询执行)                                            │
│                                                                              │
│  R4 (FieldMapper 节点) ← 独立节点，使用 SemanticMapper                       │
│       └─> R4.1 (维度层级 RAG 增强)                                           │
│  R4 ← R5 (元数据工具提供字段索引数据)                                         │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
