...
后端使用现有的 `get_datasource_luid_by_name()` 函数（位于 `tableau_assistant/src/bi_platforms/tableau/metadata.py`）进行转换，支持：
- 精确匹配数据源名称
- 支持 `名称 | 项目 : 项目名` 格式
- 模糊匹配作为兜底
...
| Insight | `insights[].type` | **InsightCards** | 图标（💡/⚠️/✅） |
