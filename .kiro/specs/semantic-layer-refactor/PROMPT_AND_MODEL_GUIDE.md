...
### Prompt 与 Schema 协同工作的完整流程

以下是 LLM 使用 Prompt 和 Schema 生成结构化输出的完整流程：

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Prompt + Schema 协同工作流程                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  阶段 1: 上下文构建                                                          │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  System Message = Prompt.role + Prompt.task + Prompt.domain_knowledge│    │
│  │                 + Prompt.constraints + Schema.json_schema            │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              ↓                                               │
│  阶段 2: 用户输入                                                            │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  User Message = Prompt.user_template.format(question="各省份销售额") │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              ↓                                               │
│  阶段 3: LLM 推理（内部过程，不可见）                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  LLM 根据 Prompt 中的 CoT 步骤进行推理:                              │    │
│  │  Step 1 (intent): "用户想按省份分组查看销售额"                       │    │
│  │  Step 2 (entities): "识别到'省份'和'销售额'"                         │    │
│  │  Step 3 (roles): "'各省份'表示分组，'销售额'需要聚合"                │    │
│  │  ...                                                                 │    │
│  │                                                                      │    │
│  │  注意: 这个推理过程是隐式的，除非 Schema 中有 reasoning 字段         │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              ↓                                               │
│  阶段 4: 逐字段生成（关键阶段）                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  生成 "type" 字段时:                                                 │    │
│  │  ├─ 参考 Schema.type.description 中的 <decision_rule>               │    │
│  │  ├─ 参考 Prompt 中的领域知识（分析类型关键词映射）                   │    │
│  │  ├─ 参考用户输入（"各省份销售额"没有累计/排名关键词）                │    │
│  │  └─ 决定: 不填写 type（这是简单查询，不需要派生计算）                │    │
│  │                                                                      │    │
│  │  生成 "measures" 字段时:                                             │    │
│  │  ├─ 参考 Schema.measures.description 中的 <what>、<examples>        │    │
│  │  ├─ 参考用户输入（"销售额"是数值概念）                               │    │
│  │  └─ 决定: [{"name": "销售额", "aggregation": "sum"}]                 │    │
│  │                                                                      │    │
│  │  生成 "dimensions" 字段时:                                           │    │
│  │  ├─ 参考 Schema.dimensions.description 中的 <what>、<examples>      │    │
│  │  ├─ 参考用户输入（"各省份"表示分组）                                 │    │
│  │  └─ 决定: [{"name": "省份", "is_time": false}]                       │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              ↓                                               │
│  阶段 5: 输出验证                                                            │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  Pydantic 验证:                                                      │    │
│  │  ├─ 类型检查: measures 是 List[MeasureSpec]? ✓                      │    │
│  │  ├─ 约束检查: measures 至少有一个元素? ✓                            │    │
│  │  └─ model_validator: 依赖关系是否满足? ✓                            │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              ↓                                               │
│  最终输出: SemanticQuery 实例                                                │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

**关键洞察**：
1. **Prompt 的作用**：在阶段 3 引导 LLM 的整体推理方向
2. **Schema 的作用**：在阶段 4 指导每个字段的具体填写
3. **桥梁机制**：Schema 中的 `<decision_rule>` 将 Prompt 中的抽象思考转化为具体填写动作
4. **验证机制**：Pydantic Validator 在阶段 5 提供代码级 100% 可靠的验证
...
| 填写规则 | ❌ | ✅ | 什么时候填、怎么填 |
| 依赖关系 | ❌ | ✅ | 字段之间的依赖 |
| **决策规则** | ⚠️ 部分 | ⚠️ 部分 | **见下方"思考与填写的桥梁"** |

### 思考与填写的桥梁（前沿研究更新）

**核心问题**：Prompt 中的思考步骤如何帮助 Schema 中的字段填写？

**LLM 生成机制**：LLM 是逐 token 生成的，不是"先思考后填写"。在生成每个字段值时，LLM 会参考：
1. System Prompt（全局上下文）
2. **Schema Description（字段描述，注意力最高！）**
3. User Input（用户问题）
4. 已生成的 tokens（包括之前填写的字段值）

**解决方案**：在 Schema 的 `<decision_rule>` 中建立"思考"与"填写"的映射关系。
...
│  "Step 2: 判断这个维度是否需要计数？"                                        │
│                                                                              │
│                    ↓ 桥梁：Schema 中的 <decision_rule>                       │
│                                                                              │
│  Schema 中的填写规则:                                                        │
│  <decision_rule>                                                             │
│  思考步骤（与 Prompt 中的 Step 2 对应）:                                     │
│  - 问题中是否有"多少个"、"几种"关键词？ → aggregation = "countd"             │
│  - 问题中是否有"总"、"合计"关键词？ → aggregation = "sum"                    │
│  - 问题中是否有"平均"关键词？ → aggregation = "avg"                          │
│  </decision_rule>                                                            │
│                                                                              │
│  关键：Schema 中的 <decision_rule> 将 Prompt 中的抽象思考                    │
│        转化为具体的填写指导                                                  │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
|---------|--------|--------|---------|
| 抽象思考步骤 | ✅ | ❌ | - |
| 具体决策规则 | ❌ | ✅ | `<decision_rule>` 标签 |
| 思考→填写映射 | ❌ | ✅ | 在 `<decision_rule>` 中引用 Prompt 的步骤 |

### 黄金法则

> 如果一段描述提到了具体字段名，它应该在 Schema 里；
> 如果一段描述是通用的分析方法，它应该在 Prompt 里；
> **如果需要将思考结果转化为填写动作，使用 Schema 中的 `<decision_rule>` 建立桥梁。**
