...
class SemanticChunker:
    ...
    def _chunk_by_cluster(
        ...
        for i, cluster in enumerate(clusters):
            ...
            logger.debug(f"Created cluster chunk: {cluster.label}, {cluster.size} rows, priority={priority}")
    ...
    def _chunk_by_change_point(
        ...
        # 按 chunk_id 重新排序（保持时间顺序，但优先级已设置）
        chunks.sort(key=lambda x: x.priority)
    ...
    def _chunk_by_semantic(
        ...
        使用 dimension_hierarchy 中的分类列进行分块。
        """
        chunks = []
        column_names = df.columns.tolist()
        
        # 找到合适的分组列（unique_count 在 3-20 之间）
        group_col = None
    ...
    def chunk(
        ...
        注意：推荐使用 chunk_with_priority() 获取带优先级的分块
        """
