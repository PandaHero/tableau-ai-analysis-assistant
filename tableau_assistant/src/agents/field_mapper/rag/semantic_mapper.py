...
class MappingSource(Enum):
    ...
    CACHE = "cache"           # 缓存命中
    VECTOR = "vector"         # 向量检"
    VECTOR_FAST = "vector_fast"  # 高置信度快速路"
    ...
    def get_enhanced_results(self) -> List[Dict[str, Any]]:
        ...
        **Validates: Requirements 2.5**
        ...
        Returns:
            ...
            - field_name: 字段"
            - field_caption: 字段显示"
            - relevance_score: 相关性分"(0-1)
            - retrieval_source: 检索来"(embedding/keyword/hybrid)
...
class MappingConfig:
    ...
    Attributes:
        ...
        max_alternatives: 最大备选数"
        use_two_stage: 是否使用两阶段检索（向量检"+ Rerank"
        use_hybrid: 是否使用混合检索（向量 + BM25"
        rerank_candidates: Rerank 前的候选数"
    """
    top_k: int = 10
    confidence_threshold: float = 0.5  # 低于此分数触"LLM Fallback
    high_confidence_threshold: float = 0.9  # 高于此分数直接返"
    max_alternatives: int = 3
    use_two_stage: bool = True  # 启用两阶段检索（中等置信度时使用 Rerank"
    use_hybrid: bool = False    # 禁用混合检索（BM25 对中英文混合效果不好，且会归一化分数）
    rerank_candidates: int = 20  # Rerank 前的候选数"
...
class SemanticMapper:
    ...
    主要功能"
    - 两阶段检索：向量检"top-K + Rerank（Requirements 2.1, 2.2"
    - 混合检索：向量 + BM25（Requirements 2.3"
    - 元数据消"
    - 高置信度快速路径（>0.9 直接返回"
    - 低置信度备选（<0.7 返回 top-3"
    - 结果增强：返"relevance scores, retrieval source, ranking position（Requirements 2.5"
    
    注意"
    - LLM 判断由任务规"agent 处理，本模块只返回检索结果和候选字"
    - 缓存由上"FieldMapperNode 通过 LangGraph SqliteStore 统一管理
    """
    ...
    def __init__(
        ...
        # cache_manager 已移除，缓存由上"FieldMapperNode 处理
        
        # 初始"Reranker（两阶段检索的第二阶段"
    ...
    def _init_retriever(self) -> None:
        ...
        if self.config.use_hybrid:
            ...
            try:
                ...
                logger.debug("已初始化混合检索器（向+ BM25")
            except Exception as e:
                logger.warning(f"初始化混合检索器失败，回退到向量检 {e}")
    ...
    def set_reranker(self, reranker: BaseReranker) -> None:
        ...
        """
        self.reranker = reranker
        logger.debug(f"已设置重排序 {type(reranker).__name__}")
    ...
    def map_field(
        ...
        # 缓存由上"FieldMapperNode 处理，这里直接进行检"
        
        # 1. 获取查询向量（记"embedding 延迟"
        ...
        # 3. 两阶段检索：第一阶段 - 向量/混合检"top-K（记"retrieval 延迟"
        ...
        # 5. 分数判断逻辑"
        # - 高置信度 (>= 0.9): 直接返回，不需"Rerank
        # - 中低置信"(< 0.9): "Rerank，找最合适的字段
        #   - Rerank 后如果置信度仍然低于阈值，FieldMapperNode 会触"LLM Fallback
        ...
        # 5.2 中低置信度："Rerank，找最合适的字段
        # 注意：即使是低置信度也要"Rerank，因为正确的字段可能在候选列表中但排名靠"
        reranked = False
        if self.config.use_two_stage and self.reranker is not None:
            ...
            try:
                ...
                # Rerank 后更"top_result，但保留原始分数用于置信度判"
                top_result = retrieval_results[0]
                ...
                logger.warning(f"Rerank 失败，使用原始结 {e}")
                retrieval_results = retrieval_results[:self.config.top_k]
        else:
            retrieval_results = retrieval_results[:self.config.top_k]
        
        # Rerank 后返回结果，使用原始 Embedding 分数作为置信"
        total_ms = int((time.time() - start_time) * 1000)
        latency.total_ms = total_ms
        ...
        return FieldMappingResult(
            term=term,
            matched_field=top_result.field_chunk.field_name,
            confidence=original_score,  # 使用原始 Embedding 分数，不"Rerank 后的虚高分数
    ...
    async def amap_field(
        ...
        **Validates: Requirements 7.4, 13.4**
        ...
        # 3. 两阶段检索：第一阶段 - 向量/混合检"top-K（异步，记录 retrieval 延迟"
        retrieval_start = time.time()
        ...
        if self.config.use_two_stage and self.reranker is not None:
            ...
            try:
                ...
                self._rerank_count += 1
                latency.rerank_ms = int((time.time() - rerank_start) * 1000)
                logger.debug(f"Rerank 完成: {len(retrieval_results)} 个结 耗时 {latency.rerank_ms}ms")
            except Exception as e:
                logger.warning(f"Rerank 失败，使用原始结 {e}")
                retrieval_results = retrieval_results[:self.config.top_k]
        else:
    ...
    async def map_fields_batch_async(
        ...
        使用 asyncio.gather 并发处理最"max_concurrency 个查询"
        ...
        Args:
            ...
            role_filter: 角色过滤
            category_filter: 类别过滤
            max_concurrency: 最大并发数（默"10"
            use_native_async: 是否使用原生异步方法（默"True"
    ...
    def _build_query(self, term: str, context: Optional[str] = None) -> str:
        ...
        只使"term 进行 Embedding 检索，不拼"context"
        context 会在 LLM Fallback 时使用，避免干扰向量检索"
        """
        # 只用 term，不拼接 context
        return term
    
    # 缓存方法已移除，由上"FieldMapperNode 通过 LangGraph SqliteStore 统一管理
    ...
    def map_field_fallback(
        ...
        """
        降级模式：直接返回向量检"top-1 结果
        
        "LLM 不可用或超时时，上层可以调用此方法获取降级结果"
        ...
        **Validates: Requirements 13.3**
        ...
        result = FieldMappingResult(
            ...
            matched_field=top_result.field_chunk.field_name,
            confidence=top_result.score,
            reasoning="LLM 降级：直接使用向量检top-1 结果",
            source=MappingSource.FALLBACK,
            latency_ms=int((time.time() - start_time) * 1000),
            retrieval_results=retrieval_results
        )
