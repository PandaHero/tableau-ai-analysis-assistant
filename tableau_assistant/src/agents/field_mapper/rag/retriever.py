...
try:
    ...
    logger.warning("jieba 未安装，将使用简单分词。安装命 pip install jieba")
...
try:
    ...
    logger.warning("rank_bm25 未安装，将使用简BM25 实现。安装命 pip install rank-bm25")
...
class KeywordRetriever(BaseRetriever):
    ...
    使用 jieba 分词 + rank_bm25 库实"BM25 检索"
    支持中英文混合检索"
    ...
    def _build_bm25_index(self) -> None:
        ...
        if BM25_AVAILABLE and self._tokenized_corpus:
            ...
            logger.debug(f"BM25 索引已构建（rank_bm25 {len(chunks)} 个文")
        else:
            logger.debug(f"BM25 索引已构建（简单实现）: {len(chunks)} 个文")
    
    def _simple_bm25_scores(self, query_tokens: List[str]) -> List[float]:
        """
        简"BM25 实现（当 rank_bm25 不可用时"
...
class HybridRetriever(BaseRetriever):
    ...
    def _rrf_fusion(
        ...
        参" https://www.elastic.co/guide/en/elasticsearch/reference/current/rrf.html
        
        Args:
            embedding_results: 向量检索结"
            keyword_results: 关键词检索结"
...
def _create_reranker(
    ...
    if reranker_type == "default":
        ...
        try:
            ...
            logger.warning(f"创建 LLMReranker 失败，使用默认排 {e}")
            return DefaultReranker(top_k)
    else:
        logger.warning(f"未知的重排序器类 {reranker_type}，使用默认排")
        return DefaultReranker(top_k)
