...
class AssemblerConfig:
    ...
    Attributes:
        chunk_strategy: 分块策略
        embedding_provider: Embedding 提供者名"(zhipu/mock)
        index_dir: 索引存储目录
        use_cache: 是否使用向量缓存
        max_samples: 最大样本值数"
...
class KnowledgeAssembler:
    ...
    def __init__(
        ...
        Args:
            datasource_luid: 数据"LUID（用作命名空间）
            config: 组装器配"
            embedding_provider: 自定"Embedding 提供者（优先于配置）
        """
        ...
        try:
            ...
            logger.warning(f"无法获取 FieldIndexCache，将不使用缓 {e}")
    ...
    def load_metadata(
        ...
        优先"SqliteStore 缓存加载，如果缓存命中且元数据未变化则跳过重建"
            ...
            if cached_data:
                ...
                if cached_hash == current_hash:
                    ...
                    if self._indexer.restore_from_cache(cached_data):
                        ...
                        logger.info(
                            f"SqliteStore 缓存恢复索引: {len(self._chunks)} 个字 "
                            f"策略: {self.config.chunk_strategy.value}"
                        )
                        return len(self._chunks)
                else:
                    logger.info(f"元数据已变化，需要重建索(cached_hash={cached_hash[:8]}..., current_hash={current_hash[:8]}...)")
        ...
        logger.info(
            f"元数据加载完 {len(fields)} 个字" 
            f"{chunk_count} 个分 策略: {self.config.chunk_strategy.value}"
        )
    ...
    def _create_merged_field(
        ...
        merged = FieldMetadata(
            ...
            sample_values=field_names[:10],  # 使用字段名作为样本"
        )
    ...
    def as_retriever(
        ...
        if not self._is_loaded:
            logger.warning("元数据未加载，请先调load_metadata()")
        ...
        if retriever_type == "embedding":
            ...
            if retriever_type != "hybrid":
                logger.warning(f"未知的检索器类型: {retriever_type}，使hybrid")
