...
class RetrievalSource(Enum):
    """检索来"""
    EMBEDDING = "embedding"  # 向量检"
    KEYWORD = "keyword"      # 关键词检"
    HYBRID = "hybrid"        # 混合检"
...
class EmbeddingResult:
    ...
    def __post_init__(self):
        if len(self.vector) != self.dimensions:
            raise ValueError(
                f"向量维度不匹 期望 {self.dimensions}, 实际 {len(self.vector)}"
            )
...
class RetrievalResult:
    ...
    def __post_init__(self):
        if not 0 <= self.score <= 1:
            raise ValueError(f"分数必须0-1 之间: {self.score}")
        if self.rank < 1:
