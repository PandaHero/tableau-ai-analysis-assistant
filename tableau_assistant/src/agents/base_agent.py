...
class BaseVizQLAgent:
    ...
    async def _execute_with_prompt(
        ...
        from tableau_assistant.src.utils.llm_cache import LLMCache
        ...
        if model_config:
            ...
            # Default: use settings configuration
            # provider: 模型提供商（openai, local, azure等）
            # model_name: 具体的模型名称（deepseek-chat, gpt-4等）
    ...
    def _basic_json_fix(self, content: str) -> str:
        ...
        # This handles missing array wrapper when LLM outputs multiple arrays
        content = re.sub(r'(\]\s*\])\s*,\s*(\[\s*\[)', r'],\2', content)
        
        # Fix nested array issue: "subtasks": [[{...}], [{...}]] -> "subtasks": [{...}, {...}]
        # This handles the case where LLM wraps each subtask in an extra array
        content = re.sub(r'(\[)\s*\[\s*(\{)', r'\1\2', content)  # Remove opening [[ before {
        content = re.sub(r'(\})\s*\]\s*,\s*\[\s*(\{)', r'\1,\2', content)  # Replace }], [{ with },{
        content = re.sub(r'(\})\s*\]\s*(\])', r'\1\2', content)  # Remove closing ]] after }
