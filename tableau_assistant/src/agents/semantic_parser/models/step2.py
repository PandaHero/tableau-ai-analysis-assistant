...
class Step2Validation(BaseModel):
    ...
    <fill_order>
    1. target_check
    2. partition_by_check (for table calcs) or dimensions_check (for LOD)
    3. calc_type_check
    4. all_valid
...
<when>ALWAYS</when>"""
    ...
    partition_by_check: ValidationCheck = Field(
        description="""<what>Check partition_by/dimensions subset of where.dimensions</what>
<when>ALWAYS</when>
<rule>For table calcs: check partition_by; For LOD: check dimensions</rule>"""
    )
