...
class MetadataManager:
    ...
    def __init__(
        ...
        Args:
            ...
            date_manager: 日期管理器（可选，用于检测STRING类型日期字段格式）
        """
    ...
    async def get_metadata_async(
        ...
        if use_cache:
            ...
            if cached_metadata:
                ...
                # 检测STRING类型日期字段格式（如果DateManager缓存为空）
    ...
    async def _detect_date_field_formats(self, metadata: Metadata) -> None:
        ...
        遍历所有STRING类型字段，使用DateManager检测其日期格式。
        如果检测成功，将格式信息缓存到DateManager中。
        
        Args:
            metadata: Metadata模型对象（会被修改）
        """
        if not self.date_manager:
            logger.debug("DateManager未注入，跳过日期格式检测")
            return
        ...
        if not string_fields:
            logger.debug("未找到STRING类型字段，跳过日期格式检测")
            return
        
        logger.info(f"开始检测 {len(string_fields)} 个STRING字段的日期格式")
    ...
    def get_field_date_format(self, field_name: str) -> Optional['DateFormatType']:
        """
        获取字段的日期格式（从DateManager缓存读取）
        
        Args:
            field_name: 字段名称
        
        Returns:
            日期格式类型，如果未检测到则返回None
        
        Examples:
            >>> format_type = metadata_manager.get_field_date_format("Order Date String")
            >>> if format_type:
            ...     print(f"字段格式: {format_type.value}")
        """
        if not self.date_manager:
            logger.debug("DateManager未注入，无法获取日期格式")
            return None
