...
class WorkflowContext(BaseModel):
    ...
    data_model: Optional[DataModel]  # 数据模型（由 DataModelCache 加载）
...
### 7.1 LangGraph SqliteStore

```python
from langgraph.store.sqlite import SqliteStore

# 获取全局单例
def get_langgraph_store() -> SqliteStore:
    """获取 LangGraph SqliteStore 单例"""
    global _store
    if _store is None:
        _store = SqliteStore.from_conn_string(f"sqlite:///{db_path}")
    return _store
...
# 通用 KV 存储
store.put(namespace=("field_mapping", datasource_luid), key=key, value={...}, ttl=86400)
item = store.get(namespace=("field_mapping", datasource_luid), key=key)
```
